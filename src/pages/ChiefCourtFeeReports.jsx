...
export default function ChiefCourtFeeReports() {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/court-fees/chief/all?court=${encodeURIComponent(selectedCourt)}`,
        {
  ...
  const markAsViewed = async (id) => {
    try {
      await axios.patch(`http://localhost:5000/api/court-fees/chief/view/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmReject = async () => {
    try {
      await axios.patch(
        `http://localhost:5000/api/court-fees/chief/reject/${rejectModal.reportId}`,
...
const generateCombinedPDF = () => {
  ...
  doc.save(`Chief_Court_Fees_Reports_${selectedCourt}.pdf`);
};
...
const generateSinglePDF = (report) => {
  ...
  doc.save(`Chief_Court_Fees_Report_${report.term}_${report.year}.pdf`);
};
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h2 className="text-center mb-4">Chief Justice Court Fees Reports - {selectedCourt}</h2>
