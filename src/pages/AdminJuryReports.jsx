...
export default function AdminJuryReports() {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    ...
    fetchReports();
    fetchUserData(); // Add this to fetch user data
  }, [selectedCourt]);
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/jury-reports/admin/all?court=${encodeURIComponent(selectedCourt)}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const markAsViewed = async (id) => {
    try {
      await axios.patch(`http://localhost:5000/api/jury-reports/view/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmReject = async () => {
    try {
      await axios.patch(
        `http://localhost:5000/api/jury-reports/admin/reject/${rejectModal.reportId}`,
...
const generateCombinedPDF = () => {
 ...
 const firstReport = validReports[0];
    const userCourt = firstReport?.court || selectedCourt || "EIGHTH JUDICIAL CIRCUIT COURT";
    const userCounty = firstReport?.county || "NIMBA COUNTY";
  ...
  validReports.forEach((report, index) => {
    // Header section with dynamic court info
    doc.setFontSize(14);
  ...
  doc.save(`All_Jury_Reports_${selectedCourt}.pdf`);
};
...
const generateSinglePDF = (report) => {
  ...
  // Get user's court and county from user data
  const userCourt = report?.court || selectedCourt || "EIGHTH JUDICIAL CIRCUIT COURT";
    const userCounty = report?.county || "NIMBA COUNTY";
  ...
  doc.save(`Jury_Report_${report.term}_${report.year}.pdf`);
};
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h2 className="text-center mb-4">Jury Payroll Reports - {selectedCourt}</h2>
