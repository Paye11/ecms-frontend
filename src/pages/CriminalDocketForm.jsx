...
export default function CriminalDocketForm() {
  ...
  const addCaseToList = () => {
    if (!currentCase.defendant || !currentCase.crime || !currentCase.dateFiled) {
      toast.error("Defendant, crime, and date filed are required");
      return;
    }
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      if (editingId) {
        ...
        navigate('/clerk/criminal-dockets');
      }
    } catch (err) {
  ...
  const confirmFinalize = async () => {
    try {
      ...
      if (editingId) {
        ...
        navigate(`/clerk/criminal-dockets?id=${res.data._id}`);
      }
    } catch (err) {
 ...
 const handleFileUpload = async () => {
  ...
  try {
    ...
    const res = await axios.post(
      '/api/criminal-dockets/upload', // Use relative path
      formData,
      {
        headers: {
...
const handleFileDelete = async (fileUrl) => {
  try {
    // Extract the relative path if full URL was stored
    const relativeUrl = fileUrl.startsWith('http')
      ? new URL(fileUrl).pathname
      : fileUrl;

    await axios.delete('/api/criminal-dockets/delete-file', {
      data: { 
        url: relativeUrl, 
 ...
 const generatePDF = () => {
    ...
    const caseTableData = form.cases.map((c, index) => [
        ...
        c.defendant,
        c.crime.toUpperCase(),
        formatDate(c.dateFiled) // Using the formatted date
    ]);
    ...
    doc.save(`CriminalDocket-${form.term}-${form.year}.pdf`);
};
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h3 className="text-center mb-4">CRIMINAL CASES DOCKET</h3>
        ...
        <div className="card-body">
          ...
          <div className="d-flex justify-content-between mt-4">
            <button 
              ...
              onClick={() => navigate('/clerk/criminal-dockets')}
            >
              Back to Dockets
            </button>
