...
export default function ChiefReports() {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/returns/chief/all?court=${encodeURIComponent(selectedCourt)}`,
  ...
  const markAsViewed = async (id) => {
    try {
      await axios.patch(`http://localhost:5000/api/returns/chief/view/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmReject = async () => {
    try {
      await axios.patch(
        `http://localhost:5000/api/returns/chief/reject/${selectedReport._id}`,
  ...
  const generateCombinedPDF = () => {
    ...
    validReports.forEach((report, index) => {
      ...
      y += 8;
      doc.text(`STATUS: ${report.chiefViewed ? 'APPROVED' : 'PENDING'}`, 14, y);
    ...
    doc.save(`ChiefJustice_Returns_Reports_${selectedCourt}.pdf`);
  };
  ...
  const renderStatus = (report) => {
    ...
    if (report.chiefViewed) return <span className="text-success">Approved</span>;
  ...
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h2 className="text-center mb-4">Chief Justice Reports - {selectedCourt}</h2>
