...
export default function ChiefCivilDockets() {
...
const generateCombinedPDF = () => {
  ...
  validDockets.forEach((docket, index) => {
    ...
    doc.autoTable({
      startY: y,
      head: [["NO.", "PLAINTIFF (S)", "", "DEFENDANT (S)", "ACTION (S)", "DATE FILED"]],
      body: caseTableData,
      styles: {
        fontSize: 10,
        halign: 'center',
  ...
  doc.save(`Chief_Civil_Dockets_${selectedCourt}.pdf`);
};
...
const generateSinglePDF = (docket) => {
  ...
  doc.save(`Chief_CivilDocket_${docket.term || ''}_${docket.year || ''}.pdf`);
};
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h2 className="text-center mb-4">Chief Justice Civil Dockets - {selectedCourt}</h2>
