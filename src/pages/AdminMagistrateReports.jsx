...
export default function AdminMagistrateReports() {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/magistrate-reports/admin/all?court=${encodeURIComponent(selectedCourt)}`,
        {
  ...
  const confirmReject = async () => {
    try {
      await axios.patch(
        `http://localhost:5000/api/magistrate-reports/admin/reject/${rejectModal.reportId}`,
  ...
  const generateCombinedPDF = () => {
    ...
    doc.save(`All_Magistrate_Reports_${selectedCourt}.pdf`);
  };
  ...
  const generateSinglePDF = (report) => {
    ...
    doc.save(`Magistrate_Report_${report.term}_${report.year}.pdf`);
  };

  return (
    <div className="container mt-4">
      <ToastContainer />
      <h2 className="text-center mb-4">Magistrate Reports - {selectedCourt}</h2>
