...
export default function AdminCivilDockets() {
...
const generateCombinedPDF = () => {
  ...
  validDockets.forEach((docket, index) => {
    ...
    doc.autoTable({
      ...
      didDrawPage: function(data) {
        // Add continuation header to new pages (except first page of each docket)
        if (data.pageNumber > 1 && data.pageCount > 1) {
          doc.setFontSize(10);
          doc.setFont("helvetica", "italic");
          doc.text("Civil Docket Continued...", 20, 10);
        }
      }
    });
  ...
  doc.save(`All_Civil_Dockets_${selectedCourt}.pdf`);
};
...
const generateSinglePDF = (docket) => {
  ...
  doc.autoTable({
    ...
    didDrawPage: function(data) {
      // Add continuation header to new pages (except first page)
  ...
  doc.save(`CivilDocket_${docket.term || ''}_${docket.year || ''}.pdf`);
};

  return (
    <div className="container mt-4">
      <ToastContainer />
      <h2 className="text-center mb-4">Civil Dockets - {selectedCourt}</h2>
                ...
                <table className="table table-bordered">
                  <thead>
                    <tr>
                      ...
                      <th>Plaintiff</th>
                      <th>Defendant</th>
                      <th>Action</th> {/* Changed from Crime to Action */}
                      <th>Date Filed</th>
                  ...
                  <tbody>
                    {(viewModal.docket.cases || []).map((c, index) => (
                      <tr key={index}>
                        ...
                        <td>{c.action || 'N/A'}</td> {/* Changed from crime to action */}
